  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-stretch h-20">
        {/* Home Team */}
        <div 
          className="flex items-center gap-3 px-6 flex-1 relative overflow-hidden"
          style={{
          ...
          <div className="relative flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="shrink-0" data-testid="text-home-logo">
              {gameState.homeTeam.logo.startsWith('http') || gameState.homeTeam.logo.startsWith('data:') ? (
                <img src={gameState.homeTeam.logo} alt="Home team logo" className="w-12 h-12 object-contain" />
              ) : (
                <span className="text-4xl">{gameState.homeTeam.logo}</span>
              )}
            </div>

            <div className="flex-1 min-w-0">
              <div 
                className="font-display font-black text-2xl text-white drop-shadow-lg truncate leading-tight"
                data-testid="text-home-team-name"
              >
                {gameState.homeTeam.name}
              </div>
              <div 
                className="font-display font-semibold text-sm text-white/90 drop-shadow-md"
                data-testid="text-home-record"
              >
                ({gameState.homeTeam.record})
              </div>
              <div className="flex gap-2 mt-1" data-testid="text-home-timeouts">
                {Array.from({ length: gameState.homeTeam.timeouts }).map((_, i) => (
                  <div key={i} className="w-8 h-4 bg-yellow-400 border-2 border-yellow-600" />
                ))}
              </div>
            </div>
        ...
        {/* Center Info */}
        <div className="flex items-center gap-4 px-6 bg-gradient-to-b from-gray-900 to-black border-x-2 border-white/20 shrink-0 relative z-30">
          <div 
            className={cn(
              "text-2xl font-display font-black text-white tabular-nums min-w-[4.5rem] text-center",
          ...
          <div 
            className="text-lg font-display font-semibold text-chart-2"
        ...
        {/* Away Team */}
        <div 
          className="flex items-center gap-3 px-6 flex-1 justify-end relative overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="relative flex items-center gap-3 flex-1 min-w-0 justify-end">
            <div className="relative shrink-0">
              <div
                className={cn(
                  "text-5xl font-display font-black text-white tabular-nums drop-shadow-2xl transition-all duration-300",
            ...
            <div className="flex-1 min-w-0 text-right">
              ...
              <div className="flex gap-2 mt-1 justify-end" data-testid="text-away-timeouts">
            ...
            <div className="shrink-0" data-testid="text-away-logo">
              {gameState.awayTeam.logo.startsWith('http') || gameState.awayTeam.logo.startsWith('data:') ? (
                <img src={gameState.awayTeam.logo} alt="Away team logo" className="w-12 h-12 object-contain" />
              ) : (
                <span className="text-4xl">{gameState.awayTeam.logo}</span>
              )}
            </div>
